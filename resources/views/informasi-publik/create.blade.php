...
<div class="max-w-3xl space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-border pb-4">
        <div>
            <h1 class="text-2xl font-display font-bold text-text-primary">Tambah Informasi Baru</h1>
            <p class="text-xs text-text-secondary mt-1">Buat pengumuman, berita, atau agenda kegiatan warga RW 047</p>
        </div>
    ...
    <div class="bg-surface p-6 rounded-md border border-border shadow-xs">
        <form method="POST" action="{{ route('informasi-publik.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="judul" name="judul" value="{{ old('judul') }}" required maxlength="150"
                    placeholder="Contoh: Jadwal Kerja Bakti Kebersihan Lingkungan RW 047"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori" required
                        ...
                        <option value="{{ $kat->value }}" {{ old('kategori') === $kat->value ? 'selected' : '' }}>
                            {{ $kat->label() }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {{-- Tanggal Agenda (Khusus Kategori Agenda) --}}
                <div id="agenda-date-container">
                    ...
                    <input type="date" id="tanggal_agenda" name="tanggal_agenda" value="{{ old('tanggal_agenda') }}"
            ...
            <div>
                ...
                <textarea id="konten" name="konten" rows="8" required
                    placeholder="Tuliskan isi pengumuman atau rincian kegiatan di sini..."
                    class="w-full text-xs bg-background border @error('konten') border-danger @else border-border @enderror rounded-sm p-3 focus:ring-1 focus:ring-primary focus:border-primary leading-relaxed">{{ old('konten') }}</textarea>
                @error('konten')
                <p class="text-[11px] text-danger mt-1">{{ $message }}</p>
                @enderror
            </div>
