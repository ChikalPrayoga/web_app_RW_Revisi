...
> **Relational Database Management System (RDBMS) — MySQL 8+ (Target Runtime Aplikasi Aktual) & SQLite in-memory (Automated Testing PHPUnit)**

### 1.2 Alasan Pemilihan

| Pertimbangan | Penjelasan |
|---|---|
| **Sifat data sangat terstruktur & relasional** | Domain sistem (kependudukan, persuratan, keuangan, laporan) memiliki relasi antarentitas yang jelas dan tetap (KK ↔ Warga, Surat ↔ Warga, Iuran ↔ KK ↔ Jenis Iuran, Kas Keluar ↔ User) — cocok dimodelkan sebagai skema relasional dengan foreign key yang tegas, bukan dokumen bebas skema (NoSQL). |
| **Kebutuhan integritas & konsistensi transaksional (ACID)** | Proses seperti verifikasi surat berjenjang, pencatatan iuran warga, dan persetujuan pengeluaran kas memerlukan jaminan transaksi atomik (mis. update status + catat audit log harus konsisten) — RDBMS unggul di sini dibanding model *eventual consistency* pada banyak database NoSQL. |
| **Kebutuhan query relasional kompleks** | Laporan keuangan (rekap iuran per RT/periode, rekap kas keluar, dan saldo akhir), serta dashboard monitoring memerlukan JOIN antar tabel serta agregasi (`SUM`, `COUNT`, `GROUP BY`) — jauh lebih alami dan efisien di RDBMS. |
| **Kebutuhan enkripsi kolom & pencarian presisi (hash index)** | Pola enkripsi AES-256 untuk NIK/No. KK dengan kolom hash pendamping (HMAC-SHA256) untuk pencarian presisi memerlukan tipe kolom dan indeks unik yang eksplisit — didukung penuh oleh MySQL 8+. |
| **Evolusi dari sistem acuan** | Sistem acuan (skripsi) sudah menggunakan RDBMS (SQLite) dengan skema ERD/LRS yang matang. Migrasi ke MySQL mempertahankan model data yang sudah teruji, meningkatkan kapabilitas concurrency dan skalabilitas untuk kebutuhan produksi. |

### 1.3 Database Target Aktual & Testing

| Environment | Database Engine | Driver Laravel | Catatan |
|---|---|---|---|
| **Runtime Aplikasi (Local/Production)** | MySQL 8+ | `mysql` | Target utama implementasi aplikasi (`DB_CONNECTION=mysql` pada `.env`) |
| **Automated Testing (PHPUnit/Feature Tests)** | SQLite (In-Memory `:memory:`) | `sqlite` | Eksekusi test cepat dan terisolasi (`phpunit.xml`) |

### 1.4 Perubahan Kunci dari Sistem Acuan

| Sistem Acuan (Skripsi) | Rebuild |
|---|---|
| SQLite (file-based, single-writer) | MySQL (server-based, mendukung concurrency multi-user) / SQLite untuk testing |
| Tipe `TEXT` untuk kolom terenkripsi (menghindari truncation Base64) | Tetap dipertahankan — `TEXT` untuk ciphertext AES-256 |
| Tidak ada tabel Role terpisah (role disebut implisit) | Ditambahkan tabel `roles` & `permissions` eksplisit (RBAC custom terstruktur) |
...
| 10 | `catatan_iurans` | Keuangan | Transaksi pencatatan pembayaran iuran warga (kas masuk) |
| 11 | `kas_keluars` | Keuangan | Transaksi pencatatan pengeluaran kas RW (kas keluar) |
| 12 | `informasi_publiks` | Informasi Publik | Pengumuman, berita, agenda kegiatan RW |
| 13 | `audit_logs` | Pendukung | Log audit lintas modul (siapa, kapan, aksi apa) |
...
|---|---|---|---|---|
| `id` | BIGINT | **PK**, GENERATED BY DEFAULT AS IDENTITY | — | Surrogate primary key native PostgreSQL |
| `no_kk` | TEXT | NOT NULL | AES-256-CBC Encrypted | Nomor KK 16-digit, terenkripsi (Encrypted PII Payload) |
| `no_kk_hash` | VARCHAR(64) | NOT NULL, UNIQUE INDEX | HMAC-SHA256 Hash | Hash deterministik untuk pencarian presisi & pencegahan duplikasi |
...
|---|---|---|---|---|
| `id` | BIGINT | **PK**, GENERATED BY DEFAULT AS IDENTITY | — | Surrogate primary key native PostgreSQL |
| `kartu_keluarga_id` | BIGINT | **FK** → `kartu_keluargas.id`, NOT NULL, INDEX | — | Referensi ke ID Kartu Keluarga tempat warga terdaftar |
| `nik` | TEXT | NOT NULL | AES-256-CBC Encrypted | Nomor Induk Kependudukan 16-digit, terenkripsi (Encrypted PII Payload) |
| `nik_hash` | VARCHAR(64) | NOT NULL, UNIQUE INDEX | HMAC-SHA256 Hash | Hash deterministik NIK untuk pencarian data & pencegahan duplikasi |
| `no_kk` | TEXT | NOT NULL | AES-256-CBC Encrypted | Nomor KK terenkripsi (Encrypted PII Payload) |
...
| `tempat_lahir` | VARCHAR(255) | NOT NULL | AES-256-CBC Encrypted | Kota/Kabupaten tempat lahir, terenkripsi |
...
| `verification_status` *(baru)* | ENUM('MENUNGGU_VERIFIKASI','TERVERIFIKASI','DITOLAK') | NOT NULL, DEFAULT `MENUNGGU_VERIFIKASI` | — | Status verifikasi data warga oleh Sekretaris RW (lihat PRD FR-02, USER_STORIES.md US-KEP-01/02) |
...
> **Catatan migrasi:**
> 1. Kolom `verification_status`, `verified_by_user_id`, dan `verification_notes` adalah **penambahan** terhadap skema sistem acuan, diperlukan untuk mendukung alur verifikasi data warga berjenjang (Ketua RT input → Sekretaris RW verifikasi) yang dijelaskan pada USER_STORIES.md.
> 2. Relasi keluarga dihubungkan secara fisik via `kartu_keluarga_id` (FK → `kartu_keluargas.id`). Kolom denormalized `no_kk_hash` tidak disimpan di tabel `wargas` karena relational FK sudah tersedia, hash parent sudah terindeks, dan untuk menghindari beban sinkronisasi serta risiko data mismatch.
...
| Field Name | Data Type | Constraints | Description |
|---|---|---|---|
| `pengajuan_id` | BIGINT | **PK**, GENERATED BY DEFAULT AS IDENTITY, NOT NULL | ID unik pengajuan surat |
| `tracking_code` | VARCHAR(64) | UNIQUE, NOT NULL, INDEX | Kode unik pelacakan surat untuk pemohon publik |
| `warga_id` | BIGINT | **FK** → `wargas.id`, NOT NULL, INDEX | Referensi ke ID warga pemohon surat (diperoleh via lookup hash NIK saat public submission) |
...
| `catatan_penolakan` | TEXT | NULLABLE | Alasan/catatan penolakan jika status pengajuan `REJECTED` (dapat dilihat warga via tracking) |
...
> **Catatan desain persuratan:**
> 1. Pemohon dihubungkan via `warga_id` (FK → `wargas.id`) sebagai identitas relasional resmi tanpa menyimpan NIK plaintext di tabel ini. Saat warga mengajukan surat via Portal Warga (tanpa login), NIK yang diinput di-hash menggunakan HMAC-SHA256 untuk mencari `warga_id` yang sesuai.
> 2. `tracking_code` berfungsi sebagai bearer token / identifier publik bagi pemohon untuk memantau progres tanpa mengekspos data pribadi (PII).
> 3. Kolom `catatan_penolakan` mencatat alasan saat pengajuan ditolak di tingkat RT maupun RW, dan dapat diakses pemohon melalui fitur tracking publik.
...
| Field Name | Data Type | Constraints | Security Spec | Description |
| Field Name | Data Type | Constraints | Security / Privacy | Description |
|---|---|---|---|---|
| `aspirasi_id` | BIGINT | **PK**, AUTO_INCREMENT, NOT NULL | — | ID unik utama laporan pengaduan |
| `ticket_number` | VARCHAR(64) | UNIQUE, NOT NULL, INDEX | — | Nomor tiket unik pelacakan pengaduan warga (`LPR-YYYYMMDD-XXXXX`) |
| `warga_id` | BIGINT | **FK** → `wargas.id`, NULLABLE, INDEX | — | Referensi ke ID warga pelapor (NULL jika anonim) |
...
| `current_status` | ENUM('SUBMITTED','IN_PROGRESS','RESOLVED','CLOSED') | NOT NULL, DEFAULT `SUBMITTED` | — | Status penanganan laporan terkini |
| `catatan_tindak_lanjut` | TEXT | NULLABLE | — | Catatan/tanggapan tindak lanjut pengurus |
...
> **Catatan Penyelarasan Scope Final:** Kolom `kategori_ai` dan `skor_prioritas_ai` serta status `CLASSIFIED` pada rancangan awal berstatus **OBSOLETE / OUT-OF-SCOPE** untuk implementasi v1 (mengacu `PROJECT_SCOPE_BOUNDARIES.md`). Skema implementasi aktual menggunakan state machine `SUBMITTED → IN_PROGRESS → RESOLVED → CLOSED`, `warga_id` nullable (mendukung pengaduan anonim), dan field `catatan_tindak_lanjut` (TEXT, NULLABLE) untuk dokumentasi penanganan pengurus.
...
| Field Name | Data Type | Constraints | Security Spec | Description |
|---|---|---|---|---|
| `iuran_id` | BIGINT | **PK**, GENERATED BY DEFAULT AS IDENTITY, NOT NULL | — | ID unik pencatatan iuran warga |
| `kartu_keluarga_id` | BIGINT | **FK** → `kartu_keluargas.id`, NOT NULL, INDEX | — | Referensi ke ID Kartu Keluarga pembayar iuran |
...
> **Constraint Anti-Duplikasi Iuran Aktif** *(keputusan arsitektur final)*: `UNIQUE (kartu_keluarga_id, iuran_type_id, periode_bulan, periode_tahun)` untuk transaksi aktif (`status IN ('PENDING', 'APPROVED')` dan `deleted_at IS NULL`). Baris berstatus `REJECTED` dan soft-deleted dikecualikan agar Ketua RT dapat mencatat ulang transaksi yang ditolak/dibatalkan.
> - **Implementasi MySQL 8+ (Runtime Aplikasi):** Menggunakan *stored generated column* `guard_col TINYINT GENERATED ALWAYS AS (CASE WHEN deleted_at IS NULL AND status IN ('PENDING', 'APPROVED') THEN 1 ELSE NULL END) STORED` dengan composite unique index `(kartu_keluarga_id, iuran_type_id, periode_bulan, periode_tahun, guard_col)`. Karena `NULL != NULL` di MySQL, baris `REJECTED` (`guard_col = NULL`) tidak memicu duplikasi.
> - **Implementasi SQLite (Automated Testing PHPUnit):** Menggunakan native partial unique index `CREATE UNIQUE INDEX uq_catatan_iuran_active ON catatan_iurans (kartu_keluarga_id, iuran_type_id, periode_bulan, periode_tahun) WHERE status != 'REJECTED' AND deleted_at IS NULL`.
> - **Lapisan Pertahanan Aplikasi:** Service Layer (`KeuanganService`) melakukan *pre-check* sebelum insert untuk menghasilkan respons `409 Conflict` yang ramah pengguna, sementara database unique index menjadi *final race-condition guard*.

---

### 3.11 Tabel `kas_keluars` *(baru — Modul Keuangan: Pengeluaran Kas RW)*

Menyimpan data transaksi pengeluaran kas RW (kas keluar) yang dicatat oleh Bendahara RW dan diverifikasi oleh Ketua RW (menegakkan prinsip *dual-control* pengeluaran dana). Tidak memiliki relasi ke KK/Warga karena merupakan transaksi operasional lingkungan RW.

| Field Name | Data Type | Constraints | Security Spec | Description |
|---|---|---|---|---|
| `id` | BIGINT | **PK**, AUTO INCREMENT, NOT NULL | — | ID unik pencatatan pengeluaran kas |
| `kategori` | VARCHAR(100) | NOT NULL | — | Kategori pengeluaran (mis. Kebersihan Lingkungan, Operasional Pos RW, Perbaikan Fasilitas) |
| `keterangan` | TEXT | NOT NULL | — | Rincian keperluan pengeluaran dana |
| `nominal` | DECIMAL(12,2) | NOT NULL | — | Jumlah dana kas yang dikeluarkan |
| `tanggal_pengeluaran` | DATE | NOT NULL | — | Tanggal aktual pengeluaran kas terjadi |
| `bukti_path` | VARCHAR(255) | NULLABLE | — | Path berkas/foto bukti kuitansi/struk pengeluaran |
| `recorded_by_user_id` | INT | **FK** → `users.id`, NOT NULL | — | ID Bendahara RW yang mencatat pengeluaran |
| `status` | ENUM('PENDING','APPROVED','REJECTED') | NOT NULL, DEFAULT `PENDING` | — | Status persetujuan pengeluaran dana RW |
| `approved_by_user_id` | INT | **FK** → `users.id`, NULLABLE | — | ID Ketua RW yang menyetujui/menolak pengeluaran |
| `approved_at` | DATETIME | NULLABLE | — | Timestamp saat disetujui/ditolak Ketua RW |
| `rejection_notes` | TEXT | NULLABLE | — | Alasan penolakan jika pengeluaran ditolak Ketua RW |
| `created_at` | TIMESTAMP | NULLABLE | — | Catatan waktu pembuatan record |
| `updated_at` | TIMESTAMP | NULLABLE | — | Catatan waktu pembaruan record |
| `deleted_at` | TIMESTAMP | NULLABLE | SoftDeletes | Catatan waktu penghapusan logis |

> **Catatan:** Transaksi kas keluar tidak memerlukan constraint anti-duplikasi karena pengeluaran operasional dengan kategori dan periode yang sama dapat terjadi berulang kali secara sah.

---

### 3.12 Tabel `informasi_publiks` *(disederhanakan dari deskripsi FR-06 PRD)*
...
### 3.13 Tabel `audit_logs` *(baru — penguatan Auditability, NFR-07 pada PRD)*
...
| `kartu_keluargas` | → | `wargas` | One-to-Many | `wargas.kartu_keluarga_id` |
| `wargas` | → | `pengajuan_surats` | One-to-Many | `pengajuan_surats.warga_id` |
| `wargas` | → | `laporan_aspirasis` | One-to-Many | `laporan_aspirasis.warga_id` |
| `kartu_keluargas` | → | `catatan_iurans` | One-to-Many | `catatan_iurans.kartu_keluarga_id` |
| `iuran_types` | → | `catatan_iurans` | One-to-Many | `catatan_iurans.iuran_type_id` |
| `users` | → | `catatan_iurans` (pencatat: Ketua RT) | One-to-Many | `catatan_iurans.recorded_by_user_id` |
| `users` | → | `catatan_iurans` (penyetuju: Bendahara RW) | One-to-Many | `catatan_iurans.approved_by_user_id` |
| `users` | → | `kas_keluars` (pencatat: Bendahara RW) | One-to-Many | `kas_keluars.recorded_by_user_id` |
| `users` | → | `kas_keluars` (penyetuju: Ketua RW) | One-to-Many | `kas_keluars.approved_by_user_id` |
...
Satu `kartu_keluargas` memiliki banyak `wargas` (One-to-Many) via `wargas.kartu_keluarga_id` — mencerminkan struktur nyata satu KK berisi beberapa anggota keluarga. Relasi ini menjadi **entitas induk** bagi hampir seluruh modul layanan lain, karena hampir semua transaksi (surat, laporan, iuran) pada akhirnya tertaut ke `wargas` dan/atau `kartu_keluargas` via surrogate key.

**c. Domain Persuratan**
Satu `wargas` dapat mengajukan banyak `pengajuan_surats` sepanjang waktu (One-to-Many) via `pengajuan_surats.warga_id` — riwayat pengajuan surat warga tersimpan penuh, bukan hanya status terkini.

**d. Domain Laporan & Aspirasi**
Satu `wargas` dapat mengirim banyak `laporan_aspirasis` (One-to-Many) via `laporan_aspirasis.warga_id` (atau nullable untuk pengaduan anonim). Setiap laporan memiliki siklus status independen (`SUBMITTED → IN_PROGRESS → RESOLVED → CLOSED`), dan penanganan tindak lanjut pengurus dicatat langsung pada field `catatan_tindak_lanjut`.

**e. Domain Keuangan (Iuran & Kas)**
Modul keuangan mencakup dua instrumen transaksi:
1. `catatan_iurans` (kas masuk): Menghubungkan `kartu_keluargas` (pembayar via `kartu_keluarga_id`), `iuran_types` (jenis iuran), dan `users` (pencatat: Ketua RT, penyetuju: Bendahara RW).
2. `kas_keluars` (kas keluar): Mencatat pengeluaran dana operasional RW oleh Bendahara RW (`recorded_by_user_id`) dan disetujui oleh Ketua RW (`approved_by_user_id`).
3. Rekapitulasi gabungan: Saldo akhir dihitung dari `SUM(catatan_iurans.nominal WHERE status='APPROVED') - SUM(kas_keluars.nominal WHERE status='APPROVED')`.
...
erDiagram
    ...
    USERS ||--o{ KAS_KELUARS : "mencatat/menyetujui"
    ...
    KARTU_KELUARGAS {
        bigint id PK
        text no_kk
        string no_kk_hash UK
        string rt_code
    }
    WARGAS {
        bigint id PK
        bigint kartu_keluarga_id FK
        text nik
        string nik_hash UK
        text no_kk
        string nama_lengkap
...
- Kolom **hash pendamping** data terenkripsi (`nik_hash`, `no_kk_hash`) **wajib** diindeks `UNIQUE` untuk mendukung pencarian presisi dan pencegahan duplikasi tanpa perlu mendekripsi data (menghindari full-table scan pada kolom ciphertext yang tidak bisa diindeks bermakna).
...
| `kartu_keluargas` | `no_kk_hash` | UNIQUE INDEX | Pencarian presisi No. KK tanpa dekripsi & pencegahan duplikasi |
| `kartu_keluargas` | `rt_code` | INDEX | Filter data KK per wilayah RT (dasar area scoping) |
| `wargas` | `kartu_keluarga_id` | INDEX (FK) | Relasi cepat ke data Kartu Keluarga |
| `wargas` | `nik_hash` | UNIQUE INDEX | Pencarian presisi NIK tanpa dekripsi & pencegahan duplikasi |
...
| `pengajuan_surats` | `warga_id` | INDEX (FK) | Riwayat pengajuan per warga |
...
| `laporan_aspirasis` | `warga_id` | INDEX (FK) | Riwayat laporan per warga |
| `laporan_aspirasis` | (`current_status`, `submitted_at`) | COMPOSITE INDEX | Query dashboard laporan aktif diurutkan waktu masuk |
| `catatan_iurans` | `kartu_keluarga_id` | INDEX (FK) | Riwayat pembayaran per KK |
...
| `catatan_iurans` | (`kartu_keluarga_id`, `iuran_type_id`, `periode_bulan`, `periode_tahun`, `guard_col`) *(MySQL)* / Partial Index *(SQLite)* | UNIQUE INDEX (filtered) | Menegakkan larangan duplikasi pencatatan iuran aktif (mengecualikan `REJECTED` dan soft-deleted) |
| `kas_keluars` | `recorded_by_user_id` | INDEX (FK) | Filter pencatatan pengeluaran per Bendahara RW |
| `kas_keluars` | `approved_by_user_id` | INDEX (FK) | Filter persetujuan pengeluaran per Ketua RW |
| `kas_keluars` | `status` | INDEX | Filter transaksi pengeluaran `PENDING` yang menunggu persetujuan Ketua RW |
| `kas_keluars` | `tanggal_pengeluaran` | INDEX | Agregasi dan filter laporan pengeluaran kas per tanggal/periode |
...
| **Eager loading di ORM** | Hindari N+1 query pada relasi yang sering diakses bersamaan (mis. `wargas` dengan `kartuKeluarga`) — gunakan eager loading (`with()`) di Laravel Eloquent. |
...
### 5.4 Pertimbangan Khusus: Kolom Terenkripsi & Identity Separation

Karena kolom seperti `nik`, `no_kk`, `alamat_lengkap`, `tempat_lahir`, `nomor_hp` disimpan sebagai **ciphertext** (hasil enkripsi AES-256-CBC dengan random IV):

1. **Pemisahan Peran Identitas:**
   - **Relational Identity:** Menggunakan surrogate key native PostgreSQL (`BIGINT GENERATED BY DEFAULT AS IDENTITY`) sebagai PK dan FK fisik.
   - **Encrypted PII Payload:** Ciphertext disimpan at-rest untuk perlindungan privasi data warga dan didekripsi di application layer hanya untuk peran berwenang. Ciphertext tidak pernah digunakan sebagai PK/FK atau parameter filter SQL.
   - **Deterministic Lookup Identity:** Hash HMAC-SHA256 disimpan sebagai kolom terpisah (`nik_hash`, `no_kk_hash`) berindeks `UNIQUE` untuk exact-match lookup dan pencegahan duplikasi tanpa membuka enkripsi.
2. Pola ini **wajib dipertahankan** karena menjadi satu-satunya cara pencarian presisi dan integritas relasional tanpa mengorbankan enkripsi at-rest data pribadi (selaras dengan NFR-08 Data Privacy Compliance pada PRD).

---

## 6. Catatan Migrasi (Migration Notes)

- Seluruh tabel di atas diimplementasikan sebagai Laravel Migration bertahap (per modul) menggunakan PostgreSQL native identity (`$table->id()`) dan foreign key (`$table->foreignId()`), agar riwayat perubahan skema tetap version-controlled dan dapat di-rollback.
...
- Tabel `kartu_keluargas` dan `wargas` (Sprint 5) menjadi fondasi relasional kependudukan bagi modul-modul transaksi downstream (Persuratan, Laporan, Keuangan).
